...
export default function Home() {
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (mascots) mascots.style.transform = `scale(${mascotsScale})`;
      if (codeEditor)
    ...
    <>
      ...
      <main className="bg-gradient-to-b from-[#0B1033] to-[#1A1F3D] text-white min-h-screen font-inter">
        ...
        <section className="max-w-[1280px] mx-auto px-4 mt-20 text-center">
          ...
          <p className="mt-4 text-sm sm:text-base max-w-md mx-auto">
          ...
          <form
            action="#"
            method="POST"
            className="mt-8 flex flex-col sm:flex-row justify-center items-stretch gap-3 max-w-md mx-auto"
          >
            <input
              type="email"
              required
              placeholder="Enter your email"
              className="flex-grow rounded-l-md sm:rounded-r-none sm:rounded-l-md py-2 px-4 text-gray-900 text-sm bg-gradient-to-r from-white/90 to-white/30 placeholder-gray-500 focus:outline-none"
            />
            <button
              type="submit"
              className="bg-[#2EA043] hover:bg-[#2c8c3a] text-white font-semibold text-sm py-2 px-5 rounded-r-md sm:rounded-l-none sm:rounded-r-md flex items-center justify-center"
            >
              Sign up for GitHub
            </button>
            <button
              type="button"
              className="border border-white rounded-md text-white font-semibold text-sm py-2 px-5 hover:bg-white hover:text-[#0B1033] transition flex items-center justify-center"
            >
              Try GitHub Copilot
            </button>
          </form>
        ...
        <section
          ...
          className="relative mt-20 max-w-5xl mx-auto parallax-zoom"
        >
          <Image
